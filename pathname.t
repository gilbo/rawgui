...
------------------------------------------------------------------------------
-- Reference Path Strings: Root & Working Directory                         --
------------------------------------------------------------------------------
...
PN.isvalidfilename = POSIX_valid_filename
PN.isvalidpathname = POSIX_valid_pathname
...
local function path_or_str(obj)
  ...
  elseif not PN.ispathname(obj) then
...
function Pathname.new(path_str)
  if PN.ispathname(path_str) then
    return path_str
  end
...
function Pathname:tostring()
  local str = ''
  if self:isabsolute() then str = '/' end
...
function Pathname:dirpath()
  if self:isroot() then return self end
  local pn = clone(self)
...
function Pathname:isdir()
  return 0 ~= sys.path_is_dir(self:tostring())
end
function Pathname:isfile()
